-- Staged / milestone payments — purely additive alongside the existing
-- single-invoice flow. A job either has no payment_schedules row (existing
-- flow, unchanged) or has exactly one (this feature).
...
CREATE TABLE public.payment_schedules (
  ...
  quote_id uuid REFERENCES public.issued_quotes(id),
  contractor_id uuid NOT NULL REFERENCES public.profiles(id),
  customer_id uuid NOT NULL REFERENCES public.profiles(id),

  schedule_type text NOT NULL DEFAULT 'milestone'
    CHECK (schedule_type IN ('milestone', 'percentage', 'fixed_date')),
...
CREATE POLICY "Parties can view payment schedule"
  ON public.payment_schedules FOR SELECT
  TO authenticated
  USING (contractor_id = auth.uid() OR customer_id = auth.uid());

CREATE POLICY "Contractor can create payment schedule"
  ON public.payment_schedules FOR INSERT
  TO authenticated
  WITH CHECK (contractor_id = auth.uid());

CREATE POLICY "Contractor can update payment schedule"
...
CREATE TABLE public.payment_stages (
  ...
  trigger_type text NOT NULL DEFAULT 'milestone'
    CHECK (trigger_type IN ('milestone', 'date', 'on_acceptance')),
  trigger_date date,
  ...
  UNIQUE (schedule_id, stage_number)
);

ALTER TABLE public.payment_stages ENABLE ROW LEVEL SECURITY;

-- payment_stages has no contractor_id/customer_id of its own — resolved via
-- its parent schedule. Mirrors the "owned via an intermediary" RLS idiom
-- documented in CLAUDE.md for company-scoped tables.
CREATE POLICY "Parties can view payment stages"
  ON public.payment_stages FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.payment_schedules s
      WHERE s.id = payment_stages.schedule_id
        AND (s.contractor_id = auth.uid() OR s.customer_id = auth.uid())
    )
  );

CREATE POLICY "Contractor can create payment stages"
  ON public.payment_stages FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.payment_schedules s
...
CREATE POLICY "Contractor can update payment stages"
...
-- No DELETE policy on payment_stages — audit trail, per spec.
...
-- =========================================================================
-- 3. issued_quotes.payment_schedule — the schedule as drafted at quote
-- time, before a job (and therefore payment_schedules/payment_stages rows)
-- exists. mint_job_from_quote materialises it into the real tables below.
-- =========================================================================
...
-- =========================================================================
-- 4. auto_mark_stage_paid — catches both the Stripe webhook and manual
-- payment paths, since both ultimately flip invoices.status to 'paid'.
-- =========================================================================

CREATE OR REPLACE FUNCTION public.auto_mark_stage_paid()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF NEW.status = 'paid' AND (OLD.status IS NULL OR OLD.status != 'paid') THEN
    ...
    RETURNING id, schedule_id, title INTO v_stage;
    ...
    IF v_stage.id IS NOT NULL THEN
      ...
      SELECT
        s.contractor_id,
        'Payment received',
        'Payment received for ' || v_stage.title || '.',
        'payment_stage_paid', 'job', s.job_id, false
      FROM public.payment_schedules s
      WHERE s.id = v_stage.schedule_id;
    END IF;
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER invoice_paid_marks_stage
  AFTER UPDATE ON public.invoices
  FOR EACH ROW EXECUTE FUNCTION public.auto_mark_stage_paid();

-- =========================================================================
-- 5. mint_job_from_quote — amended to materialise the quote's draft
-- payment_schedule (if any) into payment_schedules/payment_stages once the
-- job exists. Full contract restated (see 20260807110000's own note on
-- this file's convention for functions patched more than once).
--
-- A stage with trigger_type = 'on_acceptance' IS the existing deposit
-- (PaymentScheduleBuilder.tsx sets issued_quotes.deposit_required/
-- deposit_percentage/deposit_amount from stage 1 at quote-send time, so the
-- existing accept-quote/DepositPaymentDialog flow already collected it).
-- mint_job_from_quote only ever runs once that deposit is confirmed paid
-- (see its own deposit gate below) or wasn't required — so an
-- on_acceptance stage is created directly as status='paid' with no
-- invoice_id; there is no separate stage-invoice for it.
-- =========================================================================

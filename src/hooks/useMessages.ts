...
export interface JobMessage {
  ...
  sender_id: string;
  sender_role: "business" | "contractor";
  content: string;
...
export function useMessages(
  ...
  // Resolve profile id once on mount
  useEffect(() => {
    const init = async () => {
      const {
        data: { user },
  ...
  // Realtime subscription — recreated when conversationId changes
  useEffect(() => {
  ...
  const markAllRead = useCallback(async () => {
    ...
    setMessages((prev) =>
      prev.map((m) =>
        m.read_at === null && m.sender_id !== profileId
          ? { ...m, read_at: now }
          : m,
      ),
    );
  ...
  const unreadCount = profileId
    ? messages.filter(
        (m) => m.read_at === null && m.sender_id !== profileId,
      ).length
    : 0;

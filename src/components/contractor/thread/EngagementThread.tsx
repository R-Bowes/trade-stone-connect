...
async function fetchEngagementDetail(engagement: PipelineEngagement): Promise<EngagementDetail> {
  ...
  if (enquiryId) {
    const { data } = await supabase
      .from("enquiries")
      .select("id, job_description, job_type, priority, access_notes, location, budget_range, preferred_timeline, preferred_time_of_day, preferred_window_start, preferred_window_end, photo_urls, created_at")
      .eq("id", enquiryId)
      .maybeSingle();
    enquiry = data ?? null;
  }

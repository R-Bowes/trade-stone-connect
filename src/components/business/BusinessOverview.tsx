...
import { useBusinessWorkItems } from "@/hooks/useBusinessWorkItems";
import { WorkItemsList } from "@/components/shared/work/WorkItemsList";
import { BUSINESS_WORK_ITEM_STAGES } from "@/lib/workItems";
...
export function BusinessOverview({ profileId, companyId }: Props) {
  ...
  const { items: workItems, loading: workItemsLoading, error: workItemsError, refetch: refetchWorkItems } = useBusinessWorkItems(companyId, profileId);
  ...
  const load = async () => {
    ...
    const [
      ...
      // the unified work list's Quotes stage owns the actual list rendering)
      supabase
        .from("issued_quotes")
        .select("id")
  ...
  return (
    <div className="p-6 space-y-8 max-w-5xl">
      ...
      {/* Unified work list — every actionable business item, one stage line
          (Quotes → Dispatch → Approval → Work → Completion → Payment), one
          sort. Replaces the five dashboard sections and BusinessNeedsYourAction. */}
      <WorkItemsList
        items={workItems}
        loading={workItemsLoading}
        error={workItemsError}
        onRetry={refetchWorkItems}
        onNavigate={nav}
        stages={BUSINESS_WORK_ITEM_STAGES}
        stagelessHeading="Engagements needing attention"
      />
